...
This codemod transforms @resillix/utils imports to change method calls to match the new public interface.
...
import { nameToSlug } from '@resillix/utils';
...
const { strings } = require('@resillix/utils');
...
const { nameToSlug } = require('@resillix/utils');
...
const transformImports = (root: Collection, j: JSCodeshift) => {
  ...
  root.find(j.ImportNamespaceSpecifier).forEach((specifierPath) => {
    if (specifierPath.parent.value.source.value === '@resillix/utils') {
      for (const primitive of Object.keys(changes)) {
        const functions = Object.keys(changes[primitive]);
  ...
  root
    .find(j.VariableDeclarator, {
      init: {
        ...
        arguments: [{ value: '@resillix/utils' }],
      },
    })
    .forEach((path) => {
