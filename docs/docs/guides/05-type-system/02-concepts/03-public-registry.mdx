...
flowchart TB;
  ...
  %% Type System
  subgraph TypesPackage["@resillix/types"]
    %% Nodes
  ...
  %% Strapi
  subgraph StrapiPackage["@resillix/strapi"]
    %% Nodes
    Strapi["Strapi"]
    APIs[["APIs"]]
...
```ts title="@resillix/types/public/registries.ts"
...
```ts title="@resillix/types/uid.ts"
import type { Internal, Public } from '@resillix/types';
...
```ts title="@resillix/types/schema.ts"
import type { UID, Public } from '@resillix/types';
...
```ts title="@resillix/core/document-service.ts"
...
import type { Struct } from '@resillix/strapi';
...
:::note
In the context of a Strapi application, developers are strongly encouraged to use types exported by `@resillix/strapi` and not `@resillix/types`.

This is to allow having both an internal (`@resillix/types`) and a public (`@resillix/strapi`) types API.
:::
...
```ts title="@resillix/strapi/document-service.ts"
import type { UID } from '@resillix/types';
