...
When referencing resource by their UID you'll need to use the `UID` namespace exported from `@resillix/types`.
...
```typescript
import type { UID, Data } from '@resillix/types';
...
```typescript
import type { UID, Data, Schema } from '@resillix/types';
