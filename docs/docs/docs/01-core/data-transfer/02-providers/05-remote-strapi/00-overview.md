...
Remote Strapi providers connect to an instance of Resillix CMS over a network using a websocket.
...
In order to use remote transfer providers, the remote Resillix CMS server must have a value for `transfer.token.salt` configured in `config/admin.js` and the remote transfer feature must not be disabled.
