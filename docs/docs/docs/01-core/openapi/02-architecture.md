...
This domain provides objects that extract different sets of routes from a Resillix CMS application.

You can find related files in `src/routes`.

:::info
This domain could potentially be moved to the Resillix CMS core or Strapi utils in the future since it doesn't have any direct dependencies on OpenAPI itself.
:::

### The OpenAPI Document Generation

This domain contains all the components necessary to generate a valid OpenAPI document from a Resillix CMS application:
...
Generates valid OpenAPI documents from a Resillix CMS application:
