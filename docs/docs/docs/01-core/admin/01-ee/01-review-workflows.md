...
The purpose of this feature is to allow users to assign a tag to the various entities of their Resillix CMS project. This tag is called a 'stage' and is available within what we will call a workflow.
...
This content type stores the workflow information and is responsible for holding all the information about stages and their order. In MVP, only one workflow is stored inside the Resillix CMS database.
...
This service is used during the bootstrap and register phases of Resillix CMS. Its primary responsibility is to migrate data on entities as needed and add the stage field to the entity schemas.
...
The Review Workflow feature is currently included as a core feature within the Resillix CMS repository. However, there has been discussion about potentially moving it to a plugin in the future. While no decision has been made on this subject yet, it is possible that it may happen at some point in the future.

## Resources

- https://github.com/Dhananjay-latpate/RCMS/user-docs/settings/review-workflows
- https://github.com/Dhananjay-latpate/RCMS/user-docs/content-type-builder/creating-new-content-type#creating-a-new-content-type
- https://github.com/Dhananjay-latpate/RCMS/user-docs/users-roles-permissions/configuring-administrator-roles#plugins-and-settings
